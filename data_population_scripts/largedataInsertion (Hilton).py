...
# Helper function to pick a random visitor from the generated visitor data
def generate_visitor_id(visitor_data):
    return random.choice(visitor_data)[0]  # Return visitor_id from tuple
...
visitor_inserts = []
visitor_data = []  # Store visitor data for access records
for i in range(1000):
    gender = random.choices(genders, [44.75, 55.25])[0]
    country = random.choices(countries, [75.42, 3.69, 3.55, 2.81, 1.38, 13.15])[0]
    age_range = random.choices(age_groups, [8.81, 23.0, 20.92, 21.27, 16.67, 9.34])[0]
    age = random.randint(age_range[0], age_range[1])
    visitor_id = i + 1  # Explicit visitor ID
    visitor_inserts.append(
        f"INSERT INTO hilton_visitor (visitor_id, gender, country, age) VALUES ({visitor_id}, '{gender}', '{country}', {age});")
...
access_inserts = []
for i in range(2000):
    visitor_id = generate_visitor_id(visitor_data)
